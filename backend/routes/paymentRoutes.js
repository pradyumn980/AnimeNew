...
router.post("/order", isAuthenticated, async (req, res) => {
  try {
    const { amount, currency = "INR", type, itemName, metadata = {} } = req.body;

    if (!amount || !type || !itemName) {
      return res.status(400).json({ message: "amount, type, and itemName are required." });
    }
...
router.get("/orders", isAuthenticated, async (req, res) => {
  try {
    const orders = await Order.find({ userId: req.userId }).sort({ createdAt: -1 }).limit(50);
