...
import { Check, Crown, Sparkles, Zap, Shield, Star, Loader2, Play } from "lucide-react";
...
const plans = [
  {
    ...
    cta: "Current Plan", highlight: false, plan: "",
  },
  {
    id: "monthly", name: "Monthly", price: 99, period: "/month",
    description: "All premium features, cancel anytime",
    features: ["Everything in Free", ...premiumFeatures.map((f) => f.label)],
    cta: "Subscribe Monthly", highlight: false, plan: "monthly",
  },
  {
    id: "annual", name: "Annual", price: 799, period: "/year",
    ...
    cta: "Subscribe Annually", highlight: true, plan: "annual",
  },
];
...
function AnimeShowcase({ animes, loading }: { animes: JikanAnime[]; loading: boolean }) {
  if (loading) {
    return (
      <div className="flex justify-center gap-3 overflow-x-auto pb-2 mb-14 scrollbar-hide">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="shrink-0 w-28 h-40 rounded-xl bg-white/5 border border-white/10 animate-pulse" />
        ))}
      </div>
    );
  }
    ...
    >
      ...
      <div className="flex gap-3 overflow-x-auto pb-3 justify-start md:justify-center scrollbar-hide">
        {animes.map((anime, i) => (
          ...
          >
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-2">
              <p className="text-white text-[10px] font-semibold line-clamp-2 leading-tight">{anime.title}</p>
              {anime.score > 0 && (
        ...
        <div className="shrink-0 w-28 h-40 rounded-xl border border-white/10 bg-white/5 flex flex-col items-center justify-center gap-1 text-white/40">
          ...
          <span className="text-xs font-medium text-center px-2">500+<br />more</span>
        </div>
      </div>
    </motion.div>
  );
...
export default function Pricing() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${JIKAN_BASE}/top/anime?filter=airing&limit=12&page=1`);
        if (!res.ok) return;
    ...
    <div className="min-h-screen bg-gradient-to-b from-black via-[#0f172a] to-[#1a0a2e] px-4 py-12">
      ...
      <div className="text-center mb-10">
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}
          ...
          AI recommendations, exclusive wallpapers, premium themes, watchlist sync & ad-free experience.
        </motion.p>
      </div>
      ...
      <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
        {plans.map((plan, i) => (
          <motion.div key={plan.id}
            initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 * i + 0.5 }}
            ...
            <ul className="flex flex-col gap-2.5 flex-1">
              {plan.features.map((feature) => (
                <li key={feature} className="flex items-start gap-2 text-white/80 text-sm">
                  <Check className="w-4 h-4 text-green-400 mt-0.5 shrink-0" /> {feature}
                </li>
              ))}
            </ul>
            <button
              ...
              ) : user?.isPremium && plan.plan ? "✓ Active Plan" : plan.cta}
            </button>
          </motion.div>
        ))}
      </div>
