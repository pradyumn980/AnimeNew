				...
				{characters && characters.length > 0 && (
					<div className="mb-16">
						...
						<div className="relative">
							...
							<button
								onClick={() =>
									charactersRef.current?.scrollBy({
										left: -300,
										behavior: "smooth",
									})
								}
		...
		<div className="max-w-3xl mx-auto">
			...
			<div className="bg-gradient-to-br from-slate-900/60 to-slate-800/60 rounded-xl p-4 shadow-lg border border-slate-700/60 mb-4">
				...
				<form onSubmit={handleSubmit} className="space-y-3">
						...
						<div className="flex items-center gap-2 p-2 rounded-lg border border-slate-600 bg-slate-900">
							...
							<div className="flex gap-1">
								{[1, 2, 3, 4, 5].map((star) => (
									...
									>
										<Star
											className={`w-4 h-4 ${star <= rating ? "text-yellow-400 fill-yellow-400" : "text-slate-500"}`}
										/>
									</button>
								))}
							</div>
					...
					{error && (
						<div className="text-red-400 text-xs text-center bg-red-900/20 p-2 rounded-lg">
							{error}
						</div>
					)}
					{success && (
						<div className="text-emerald-400 text-xs text-center bg-emerald-900/20 p-2 rounded-lg">
							✅ Review submitted!
						</div>
					)}
				</form>
			...
			<div className="space-y-3">
				{reviews.length === 0 ? (
					...
					reviews.map((review, idx) => (
						<div
							key={idx}
							className="relative bg-gradient-to-br from-slate-800/80 to-slate-700/80 border border-slate-600 rounded-lg p-4 shadow-md hover:shadow-lg transition-all duration-300 group"
						>
									...
									<div className="flex items-center gap-2 mb-2">
										...
										<div className="flex items-center gap-1">
											{[1, 2, 3, 4, 5].map((star) => (
												<Star
													key={star}
													className={`w-3 h-3 ${star <= (review.rating || 0) ? "text-yellow-400 fill-yellow-400" : "text-slate-500"}`}
												/>
											))}
										</div>
									</div>
