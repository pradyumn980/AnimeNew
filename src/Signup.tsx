...
export default function Signup() {
	...
	const handleAvatarSignup = async () => {
		...
		try {
			await axios.post(
				"https://animenew.onrender.com/api/set-avatar",
				{
	...
	return (
		...
		>
			{showAvatarPicker ? (
				<div className="bg-white/80 backdrop-blur-sm p-8 rounded-xl shadow-2xl w-full max-w-lg animate-fade-in relative flex flex-col items-center">
					...
					<div className="grid grid-cols-2 sm:grid-cols-5 gap-4 mb-6">
						{avatars.map((avatar, idx) => (
							<img
								...
								src={avatar}
								alt={`Avatar ${idx + 1}`}
								className={`w-20 h-20 rounded-full border-4 cursor-pointer transition-all duration-200 ${selectedAvatar === avatar ? "border-orange-500 scale-110" : "border-transparent hover:border-orange-400 hover:scale-105"}`}
								onClick={() => setSelectedAvatar(avatar)}
							/>
						))}
					</div>
					<button
						className="w-full bg-orange-600 hover:bg-orange-700 text-white font-semibold py-2 px-4 rounded flex items-center justify-center disabled:opacity-50"
				...
				<form
					className="bg-white/80 backdrop-blur-sm p-8 rounded-xl shadow-2xl w-full max-w-lg animate-fade-in relative"
				...
				>
					...
					<div className="mb-3">
						...
						<input
							...
							className="mt-1 block w-full border rounded-md h-10 py-2 px-3 text-base"
							required
						/>
					</div>
					{/* Password */}
					<div className="mb-3">
						...
						<div className="relative">
							<input
								...
								onChange={handleChange}
								className={`mt-1 block w-full border rounded-md h-10 py-2 px-3 text-base${showPassword ? "" : " pr-10"}`}
								required
							/>
							<button
								type="button"
								...
								{showPassword ? (
									...
									>
										<path
											...
											d="M1.458 12C2.732 7.943 6.523 5 12 5c5.477 0 9.268 2.943 10.542 7-.7 2.157-2.243 4.018-4.542 5.292M15 12a3 3 0 11-6 0 3 3 0 016 0z"
										/>
									</svg>
								) : (
									...
									>
										<path
											strokeLinecap="round"
											strokeLinejoin="round"
											d="M13.875 18.825A10.05 10.05 0 0112 19c-5.477 0-9.268-2.943-10.542-7a10.056 10.056 0 012.908-4.568M6.634 6.634A9.956 9.956 0 0112 5c5.477 0 9.268 2.943 10.542 7a9.956 9.956 0 01-4.043 5.013M15 12a3 3 0 11-6 0 3 3 0 016 0zm-6 0a6 6 0 016 0m-9 9l18-18"
										/>
									</svg>
								)}
							</button>
