import { Loader2, Mail, MessageCircle, Twitter } from "lucide-react";
	...
	return (
		<div className="w-full max-w-xl mx-auto p-6 bg-gradient-to-br from-black/80 to-gray-900/80 rounded-xl shadow-lg border border-gray-700/40">
			...
			<p className="text-gray-400 mb-4 text-sm">
				Have feedback or questions? Reach out below!
			</p>
			...
			<form onSubmit={handleSubmit} className="space-y-3">
				...
				/>
				{errors.name && (
					<div className="text-red-400 text-xs ml-1">{errors.name}</div>
				)}
				<input
					type="email"
					name="email"
				...
				/>
				{errors.email && (
					<div className="text-red-400 text-xs ml-1">{errors.email}</div>
				)}
				<select
					name="subject"
					value={form.subject}
				...
				/>
				{errors.message && (
					<div className="text-red-400 text-xs ml-1">{errors.message}</div>
				)}
				<button
					type="submit"
