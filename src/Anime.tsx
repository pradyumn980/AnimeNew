...
function AnimeCard({
	anime,
}: { anime: Anime; onViewDetails: (anime: Anime) => void }) {
	return (
		<div className="group relative overflow-hidden rounded-lg border border-gray-800 transition-all duration-300 hover:border-red-500/50 hover:scale-[1.01] bg-[#0e0e0e] text-white hover:shadow-xl hover:shadow-red-500/20">
			...
			<div className="relative overflow-hidden">
				<img
					...
					onError={(e) => {
						...
						target.src =
							"https://via.placeholder.com/300x400/1a365d/ffffff?text=No+Image";
					}}
				/>
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-violet-500/20 via-purple-500/20 to-pink-500/20 animate-pulse"></div>
			...
			<div className="relative p-3 space-y-2">
				...
				<Link
					to={`/anime/${anime.mal_id}`}
					className="text-base font-semibold line-clamp-2 text-white group-hover:text-red-400 transition-colors duration-300 hover:underline"
				>
					{anime.title}
				</Link>
				...
				<div className="flex items-center justify-between">
					<span
						className={`px-4 py-2 text-xs font-bold rounded-full shadow-lg border transition-all duration-300 ${
			...
			<div className="relative z-10 p-6 max-w-7xl mx-auto">
				...
				{!hasSearched && (
					<div className="flex flex-col items-center justify-center py-20 text-center">
						...
						<p className="text-slate-400 text-lg mb-4">
							Search for your next favorite series
						</p>
						<p className="text-slate-500 text-sm">
							Try searching for "Naruto", "Attack on Titan", or "One Piece"
						</p>
					</div>
				)}
