...
export default function Recommendations() {
	...
	const hasPrefs =
		savedPrefs &&
		(savedPrefs.favoriteGenres?.length || savedPrefs.favoriteAnimes?.length);
	...
	return (
		<div className="min-h-screen bg-gradient-to-b from-black via-[#0f172a] to-[#1a0a2e] px-4 py-12">
			...
			<div className="text-center mb-10">
				...
				>
					Tell us your taste and our AI will find your next obsession — with
					personalised reasons just for you.
				</motion.p>
			</div>
			...
			{hasPrefs && !loading && (
				...
				>
					...
					{savedPrefs?.favoriteGenres?.slice(0, 5).map((g) => (
						<span
							key={g}
							className="text-xs bg-purple-500/20 text-purple-300 border border-purple-500/30 rounded-full px-2.5 py-0.5"
						>
							{g}
						</span>
					))}
			...
			{!loading && !recs.length && !error && (
				...
				>
					...
					<p className="text-white/40 text-sm">
						Click "Get My Recommendations" to start your taste quiz.
					</p>
				</motion.div>
			)}
			...
			{!loading && recs.length > 0 && (
				<div className="max-w-6xl mx-auto">
						...
						{recs.map((rec, i) => (
							...
							>
								...
								<div className="relative h-52 bg-gradient-to-br from-slate-800 to-slate-900 shrink-0 overflow-hidden">
									...
									{/* Year */}
									{rec.year && (
										<div className="absolute bottom-2 right-2 text-white/60 text-[10px]">
											{rec.year}
										</div>
									)}
								</div>
									...
									<div>
										...
										<div className="flex flex-wrap gap-1 mt-1.5">
											{rec.genres.slice(0, 3).map((g) => (
												<span
													key={g}
													className="text-[10px] text-purple-300 bg-purple-500/15 rounded-full px-1.5 py-0.5"
												>
													{g}
												</span>
											))}
					...
					>
						🤖 Recommendations generated by Gemini AI · Artwork from MyAnimeList
						via Jikan API
					</motion.p>
				</div>
			)}
